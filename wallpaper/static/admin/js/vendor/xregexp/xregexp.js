 ...
 * Supports -style (?P<name> as an alternate syntax to avoid issues in recent Opera (which
 * natively supports the -style syntax). Otherwise, XRegExp might treat numbered
 * backreferences to -style named capture as octals.
 */
    add(/\(\?P?<([\w$]+)>/,
        function (match) {
            if (!isNaN(match[1])) {
